...
let rec gen_ty elems fuel =
  let sub = fuel - 1 in
  let deeper =
    if fuel <= 0 then []
    else
      [
        (1, fun () -> G.return T_unit);
        (2, fun () -> gen_ty elems sub >>= fun a -> G.return (T_option a));
        ( 2,
          fun () ->
            gen_ty elems sub >>= fun a ->
            gen_ty elems sub >>= fun e -> G.return (T_result (a, e)) );
        ( 1,
          fun () ->
            gen_tys elems sub >>= fun ps ->
            gen_ty elems sub >>= fun r -> G.return (T_fn (ps, r)) );
        ( 1,
          fun () ->
            gen_tys elems sub >>= fun ps ->
            gen_ty elems sub >>= fun r -> G.return (T_async_fn (ps, r)) );
        (1, fun () -> gen_ty elems sub >>= fun a -> G.return (T_future a));
      ]
  in
  let sigs =
    match elems with
    | [] -> []
    | e0 :: es ->
        [ (1, fun () -> pick_elt e0 es >>= fun a -> G.return (T_signal a)) ]
  in
  pick_w
    (3, fun () -> G.return T_f64)
    (append
       [ (3, fun () -> G.return T_bool); (3, fun () -> G.return T_string) ]
       (append deeper sigs))

and gen_tys elems fuel = G.int_bound 2 >>= fun n -> gen_ty_list elems fuel n

and gen_ty_list elems fuel n =
  if n <= 0 then G.return []
  else
    gen_ty elems fuel >>= fun t ->
    gen_ty_list elems fuel (n - 1) >>= fun ts -> G.return (t :: ts)
  ...
  if fuel <= 0 then leaf ctx nid t
    ...
    let generic =
      [
        ...
        ( 1,
          fun () ->
            gen_tys ctx.sig_elems 1 >>= fun ps0 ->
...
and specific_prods ctx sub nid t =
  ...
  | T_option a ->
      let none_prod =
        if turbofish_safe a then [ (1, fun () -> G.return (E_none a, nid)) ]
        else []
      in
      let okp =
...
and sig_writes ctx sub nid =
  ...
  | [] -> []
  | e0 :: es ->
      [ (3, fun () -> pick_elt e0 es >>= fun elem -> write_at ctx sub nid elem) ]
...
and closure_body ctx fuel nid r =
  let ret_unit =
    if ty_eq r T_unit then
      [ (1, fun () -> G.return (E_block ([], E_return_unit), nid)) ]
    else []
  in
  pick_w
    (4, fun () -> gen_expr ctx fuel nid r)
    (append
       [
         ( 1,
           fun () ->
...
and gen_loop_body ctx fuel nid =
  ...
  pick_w
    (2, fun () -> G.return [])
    [
      (3, fun () -> G.return [ E_break ]);
      (1, fun () -> G.return [ E_continue ]);
    ]
...
and stmts_go ctx fuel nid n acc =
  if n <= 0 then G.return (rev acc, ctx, nid)
    ...
    pick_w
      ( 3,
        fun () ->
          gen_ty ctx.sig_elems 1 >>= fun pt0 ->
...
let gen_target g =
  ...
  gen_ty elems tyfuel >>= fun t ->
  gen_expr
    { env = wf_env g; in_async = false; sig_elems = elems }
